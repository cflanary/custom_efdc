...
!      TSED0(2,L) = TSED(2,L)
  ...
  ALL_LAYERS:DO LL=1,KB !go through all sediment layers so that they are properly eroded and sorted
     IF(LAYER(LL,L)==0.AND.TSED(LL,L)==0) CYCLE !if the layer is gone don't consider it
...
!     IF(LL==1.OR.LL==2)THEN !for active layer
     ...
     IF(LL>1)THEN !calculate erosion rates of deeper layers
...
!        SN10=(TAUDD(1)-TAU(L))/(TAUDD(1)-TAUDD(2)) !weighting factor 2
        ...
        IF(LL==2)THEN
           ERATEMOD(L)=SN00*BULKDENS(LL,L)*SQRT(1./SH_SCALE(L)) !linear interpolation around size class (g/cm2/s)
        ELSE
